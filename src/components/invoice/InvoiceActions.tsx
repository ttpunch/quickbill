...
export default function InvoiceActions({ invoice }: { invoice: Invoice }) {
  ...
  const menu = open ? (
    <>
      ...
      />
      <div
        className="card fixed z-50 w-52 py-1.5"
        style={{ top: menuPos.top, right: menuPos.right }}
      >
        <Link
          href={`/dashboard/invoices/${invoice.id}`}
          className="flex items-center gap-2.5 px-3.5 py-2 text-sm text-ink transition-colors hover:bg-cream"
        ...
        {invoice.status !== 'paid' && (
          <button
            ...
            className="flex w-full items-center gap-2.5 px-3.5 py-2 text-left text-sm text-ink transition-colors hover:bg-cream"
          >
            <svg className="w-4 h-4 text-brand" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="mt-1 border-t border-line pt-1">
          {!confirming ? (
            ...
            <div className="px-3.5 py-2">
              <p className="mb-2 text-xs text-muted">Delete this invoice?</p>
              <div className="flex gap-2">
                <button onClick={handleDelete} className="rounded-full bg-danger px-3 py-1 text-xs font-semibold text-cream">Yes</button>
                <button onClick={() => setConfirming(false)} className="rounded-full border border-line-strong px-3 py-1 text-xs">No</button>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div>
      <button
        ref={buttonRef}
        onClick={handleOpen}
        className="rounded-lg p-1.5 text-faint transition-colors hover:bg-cream hover:text-ink"
      >
